# 11b_osa_phenotype_null_hub_sensitivity.R
# Hub-definition sensitivity replay: identical Freedman-Lane machinery and
# selection rule as 11_osa_phenotype_null.R, but the fixed hub sets are the
# phenotype-module hubs (MASLD: phenotype_module_nominal_p_lt_0.05_sensitivity;
# OSA: adjusted_phenotype_module_nominal_p_lt_0.05_sensitivity) instead of the
# topology-only hubs. This tests whether the conditional replay conclusion is
# robust to how the fixed network step is defined.
...
spec <- read_spec()
ensure_dirs(p("outputs", "phenotype_null_hub_sensitivity"), p("logs"))
...
masld_hubs <- unique(read.csv(
  p("outputs", "network", "MASLD_phenotype_module_hubs_sensitivity.csv"),
  check.names = FALSE
)$Gene)
osa_hubs <- unique(read.csv(
  p("outputs", "osa_network", "OSA_phenotype_module_hubs_sensitivity.csv"),
  check.names = FALSE
)$Gene)
...
stopifnot(observed$common == 12958L, is.finite(observed$r))
cat(sprintf("Hub-sensitivity observed: common=%d eligible=%d selected=%d r=%.4f\n",
            observed$common, observed$eligible, observed$selected, observed$r))
...
for (i in seq_len(n_perm)) {
  ...
  if (i %% 100L == 0L || i == n_perm) {
    ...
    cat(sprintf("Hub-sensitivity null: %d/%d iterations (%.1f min elapsed)\n",
                i, n_perm, elapsed))
  }
...
summary <- data.frame(
  Analysis = "OSA_Freedman_Lane_full_selection_replay_phenotype_hub_sensitivity",
...
write_csv_atomic(iterations, p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_iterations.csv"))
write_csv_atomic(summary, p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_summary.csv"))
saveRDS(list(observed = observed, iterations = iterations, summary = summary),
        p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_distribution.rds"), compress = "xz")
...
plot_null <- function(device, file) {
  ...
  hist(iterations$SelectedPearson[valid], breaks = 50, col = "#D9EAF4",
       ...
       main = "Hub-sensitivity residual null (phenotype-module hubs)")
...
plot_null("pdf", p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_distribution.pdf"))
plot_null("png", p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_distribution.png"))
...
contract <- list(
  ...
  network_conditioning = "MASLD and OSA phenotype-module hub sets held fixed (sensitivity)",
  tail = "upper",
  add_one_correction = TRUE,
  primary_inference = as.list(summary)
)
write_json_atomic(contract, p("outputs", "phenotype_null_hub_sensitivity", "phenotype_null_hub_sensitivity_contract.json"))

stopifnot(n_valid >= 0.95 * n_perm, p_upper > 0, p_upper <= 1)
save_session_info("11b_osa_phenotype_null_hub_sensitivity")
print(summary)
cat("Hub-sensitivity residual null completed.\n")
